...
class CustomerOrderSnapshotTests(unittest.TestCase):
    def setUp(self):
        ...
        self.shop_settings = {
            ...
            "shipping_calculated_base_cents": "500",
            "shipping_calculated_per_kg_cents": "1000",
        }
        OrderService(self.db)
        ...
        result = self.commerce.register_customer(
            "Test Customer", "order@example.com", "password123"
        )
    ...
    def test_customer_order_persists_variant_and_pricing_snapshot(self):
        row, items, subtotal, shipping, tax, total = self.commerce.create_order(
            self.user_id,
            [{"productId": "product-1", "variantId": "variant-1", "quantity": 2}],
            {"address": "1 Main St", "city": "Testville", "state": "MO", "zip": "00000"},
        )
        ...
        with self.db.connect() as conn:
            saved = conn.execute(
                "SELECT product_id,variant_id,quantity,unit_price_cents,material,color FROM order_items WHERE order_id=?",
                (row["id"],),
            ).fetchone()
        self.assertEqual(dict(saved), {
