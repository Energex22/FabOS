...
class Pass18PaymentBoundaryTests(unittest.TestCase):
    def setUp(self):
        ...
        with self.db.connect() as c:
            ...
            for uid, username, account_type in users:
                c.execute(
                    "INSERT INTO users(id,username,password_hash,account_type,active) VALUES(?,?,?,?,1)",
                    (uid, username, "x", account_type),
                )
    ...
    def _insert_order(self, order_id, number, customer_id):
        with self.db.connect() as c:
            ...
            c.execute("INSERT INTO orders(%s) VALUES(%s)" % (",".join(cols), ",".join("?" for _ in cols)), tuple(values[x] for x in cols))
            c.commit()
    ...
    def _insert_invoice(self, invoice_id, number, order_id, total):
        with self.db.connect() as c:
            ...
            for name, info in columns.items():
                if name in values or info[5] == 1 or info[4] is not None or info[3] == 0:
                    continue
                if name.endswith("_at") or name == "due_at":
                    values[name] = "2026-01-01T00:00:00"
                elif name.endswith("_cents"):
                    values[name] = 0
                else:
                    values[name] = ""
            cols = [name for name in columns if name in values]
            c.execute("INSERT INTO invoices(%s) VALUES(%s)" % (",".join(cols), ",".join("?" for _ in cols)), tuple(values[name] for name in cols))
            c.commit()
    ...
    def test_customer_can_only_read_own_invoices(self):
        ...
        with self.assertRaises(PermissionError):
            self.invoices.get_for_user("customer1", "inv2")
        rows = self.invoices.list_for_user("customer1")
        self.assertEqual([row["id"] for row in rows], ["inv1"])
    ...
    def test_customer_cannot_create_or_modify_invoice(self):
        with self.assertRaises(PermissionError):
            self.invoices.create_from_order_for_user("customer1", "order1")
        with self.assertRaises(PermissionError):
            self.invoices.update_charges_for_user("customer1", "inv1", tax_cents=100)
        with self.assertRaises(PermissionError):
            self.invoices.record_payment_for_user("customer1", "inv1", 100)
        with self.assertRaises(PermissionError):
            self.invoices.void_for_user("customer1", "inv1")
    ...
    def test_payment_read_permission_is_enforced(self):
        ...
        with self.assertRaises(PermissionError):
            self.invoices.get_for_user("employee", "inv1")

    def test_application_invoice_dependencies_are_wired(self):
        self.assertIs(self.invoices.accounts, self.accounts)
        self.assertIs(self.invoices.permissions, self.permissions)
