    ...
    def test_fresh_install_reaches_latest_migration(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with db.connect() as c:
                ...
                # Every declared migration must have a unique version and be applied
                # exactly once on a fresh install.
                self.assertEqual(len(expected_versions), len(MIGRATIONS))
                latest = c.execute("SELECT MAX(version) FROM app_migrations").fetchone()[0]
                self.assertEqual(latest, max(expected_versions))
                self.assertEqual(c.execute("SELECT COUNT(*) FROM app_migrations").fetchone()[0], len(expected_versions))
