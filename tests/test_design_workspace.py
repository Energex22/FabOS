...
class DesignWorkspaceTests(unittest.TestCase):
    ...
    def test_failed_import_removes_copied_asset(self):
        with tempfile.TemporaryDirectory() as td:
            db=Database(Path(td)/"fabos.sqlite3");db.initialize();migrate(db)
            pid=str(uuid.uuid4())
            with db.connect() as c:
                c.execute("INSERT INTO products(id,name) VALUES(?,?)",(pid,"Cleanup Product"));c.commit()
            svc=DesignVaultService(db,Path(td))
            did=svc.ensure_product(pid)
            source=Path(td)/"model.stl"
            source.write_text("solid model\nendsolid model\n",encoding="utf-8")
            with patch.object(svc,"stl_meta",side_effect=ValueError("invalid model")):
                with self.assertRaises(ValueError):
                    svc.import_file(did,source)
            self.assertEqual(svc.assets(did),[])
            stored=list((Path(td)/"Design Vault").rglob("model.stl"))
