class OrderService:
    ...
    def list(self,query="",status="All",sort_column="created",descending=True,group="all"):
        ...
        elif group=="history": where.append("(o.status IN ('completed','cancelled','shipped') OR COALESCE(f.status,'') IN ('shipped','delivered','picked_up'))")
        if status and status!="All":
            requested=status.lower()
            if requested=='shipped': where.append("(o.status='shipped' OR COALESCE(f.status,'')='shipped')")
            elif requested in ('delivered','picked_up'): where.append("COALESCE(f.status,'')=?"); args.append(requested)
