...
class InvoiceService:
 ...
 def finance_summary(self,days=None):
  ...
  with self.db.connect() as c:
   ...
   row=c.execute("""SELECT COALESCE(SUM(CASE WHEN status IN ('open','partial') THEN MAX(total_cents-paid_cents,0) ELSE 0 END),0) outstanding_cents,
      COALESCE(SUM(CASE WHEN status='partial' THEN 1 ELSE 0 END),0) partial_invoices,
      COALESCE(SUM(CASE WHEN status='open' THEN 1 ELSE 0 END),0) open_invoices FROM invoices WHERE status<>'void'""").fetchone()
 ...
 def payment_history(self,limit=100,days=None):
  with self.db.connect() as c:
   ...
   return c.execute("""SELECT p.*,i.invoice_number,o.order_number,COALESCE(c.name,'No customer') customer_name
      FROM payments p JOIN invoices i ON i.id=p.invoice_id LEFT JOIN orders o ON o.id=i.order_id LEFT JOIN customers c ON c.id=o.customer_id
 ...
 def list(self,query="",status="All",sort="created",descending=True):
  ...
  sql="""SELECT i.*,COALESCE(o.order_number,'—') order_number,COALESCE(c.name,'No customer') customer_name,
   (i.total_cents-i.paid_cents) balance_cents FROM invoices i LEFT JOIN orders o ON o.id=i.order_id LEFT JOIN customers c ON c.id=o.customer_id
 ...
 def get(self,iid):
  ...
  with self.db.connect() as c:
   inv=c.execute("""SELECT i.*,o.order_number,o.quote_id,o.customer_id,c.name customer_name,c.email customer_email,c.phone customer_phone,
    (i.total_cents-i.paid_cents) balance_cents FROM invoices i LEFT JOIN orders o ON o.id=i.order_id LEFT JOIN customers c ON c.id=o.customer_id WHERE i.id=?""",(iid,)).fetchone()
 ...
 def record_payment(self,iid,amount_cents,method="",reference="",notes=""):
  ...
  with self.db.connect() as c:
   ...
   c.execute("INSERT INTO payments(id,invoice_id,amount_cents,method,reference,notes) VALUES(?,?,?,?,?,?)",(str(uuid.uuid4()),iid,amount,method,reference,notes))
   paid=int(c.execute("SELECT COALESCE(SUM(amount_cents),0) FROM payments WHERE invoice_id=?",(iid,)).fetchone()[0])
 ...
 def export_html(self,iid):
  ...
  item_rows=[]
  for x in items:
   line=int(x["quantity"] or 1)*int(x["unit_price_cents"] or 0);mat=("%s %s"%(x["material"] or "",x["color"] or "")).strip()
   item_rows.append("<tr><td>%s</td><td>%d</td><td>%s</td><td>$%.2f</td></tr>"%(html.escape(x["description"] or ""),int(x["quantity"] or 1),html.escape(mat),line/100.0))
  ...
  with self.db.connect() as c:
   shop_name=self._setting(c,"shop_name","WireVault FabOS");shop_email=self._setting(c,"shop_email","");shop_phone=self._setting(c,"shop_phone","");shop_address=self._setting(c,"shop_address","")
...
body{font-family:Arial,sans-serif;margin:42px;color:#222}h1{margin-bottom:4px}.muted{color:#666}table{width:100%%;border-collapse:collapse;margin-top:20px}th,td{padding:9px;border-bottom:1px solid #ddd;text-align:left}.summary{margin-left:auto;width:330px}.summary td{border:0}.total{font-size:20px;font-weight:bold}@media print{button{display:none}}</style></head><body>%s<h1>%s</h1>
<div class='muted'>Order %s • Due %s</div><h3>Bill To</h3><div>%s<br>%s<br>%s</div><table><thead><tr><th>Item</th><th>Qty</th><th>Material</th><th>Line Total</th></tr></thead><tbody>%s</tbody></table>
<table class='summary'><tr><td>Subtotal</td><td>$%.2f</td></tr><tr><td>Tax</td><td>$%.2f</td></tr><tr><td>Shipping</td><td>$%.2f</td></tr><tr><td>Discount</td><td>-$%.2f</td></tr><tr class='total'><td>Total</td><td>$%.2f</td></tr><tr><td>Paid</td><td>$%.2f</td></tr><tr><td><b>Balance</b></td><td><b>$%.2f</b></td></tr></table><h3>Payments</h3><table><tbody>%s</tbody></table><p>%s</p><button onclick='window.print()'>Print Invoice</button></body></html>"""%(html.escape(inv["invoice_number"]),seller,html.escape(inv["invoice_number"]),html.escape(inv["order_number"] or "—"),html.escape(inv["due_at"] or "—"),html.escape(inv["customer_name"] or ""),html.escape(inv["customer_email"] or ""),html.escape(inv["customer_phone"] or ""),"".join(item_rows),inv["subtotal_cents"]/100.0,inv["tax_cents"]/100.0,inv["shipping_cents"]/100.0,inv["discount_cents"]/100.0,inv["total_cents"]/100.0,inv["paid_cents"]/100.0,inv["balance_cents"]/100.0,payment_rows,html.escape(inv["notes"] or ""))
  path=self.export_dir/(inv["invoice_number"]+".html");path.write_text(doc,encoding="utf-8");return path

 # Pass 18 authorization boundary. Existing mutators remain available to the trusted internal UI layer;
 # API/customer-facing code should use these actor-aware methods exclusively.
 ...
 def _invoice_allowed(self,user_id,invoice_id):
  ...
  with self.db.connect() as c:
   return bool(c.execute("""SELECT 1 FROM invoices i JOIN orders o ON o.id=i.order_id WHERE i.id=? AND o.customer_id=?""",(invoice_id,customer["id"])).fetchone())
 ...
 def list_for_user(self,user_id,query="",status="All",sort="created",descending=True):
  user=self._authorized_user(user_id,"payment.read")
  rows=self.list(query,status,sort,descending)
 ...
 def payment_history_for_user(self,user_id,limit=100,days=None):
  user=self._authorized_user(user_id,"payment.read")
  rows=self.payment_history(limit,days)
  if user["account_type"]!="customer":return rows
  customer=self.accounts.customer_for_user(user_id)
  if not customer:return []
  return [row for row in rows if row["customer_id"]==customer["id"]]
