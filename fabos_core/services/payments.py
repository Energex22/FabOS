...
class PaymentProvider:
    ...
    def create_physical_payment(self, *, payment_id, amount_cents, currency, source_id, metadata):
        raise PaymentProviderNotConfigured("Physical payment is not supported by this provider")
...
class StripePaymentProvider(PaymentProvider):
    ...
    def create_checkout(self, *, payment_id, amount_cents, currency, metadata):
        ...
        fields = {
            ...
            "line_items[0][quantity]": "1", "client_reference_id": str(metadata["order_id"]),
            "metadata[payment_id]": str(payment_id), "metadata[order_id]": str(metadata["order_id"]),
        }
...
class SquarePaymentProvider(PaymentProvider):
    ...
    def __init__(self):
        self.access_token = os.environ.get("SQUARE_ACCESS_TOKEN", "").strip()
        self.location_id = os.environ.get("SQUARE_LOCATION_ID", "").strip()
        self.webhook_signature_key = os.environ.get("SQUARE_WEBHOOK_SIGNATURE_KEY", "").strip()
        self.webhook_url = os.environ.get("SQUARE_WEBHOOK_URL", "").strip()
        if not self.access_token or not self.location_id:
            raise PaymentProviderNotConfigured("SQUARE_ACCESS_TOKEN and SQUARE_LOCATION_ID are required")

    def create_checkout(self, *, payment_id, amount_cents, currency, metadata):
        raise PaymentProviderNotConfigured("Square checkout is reserved for physical sales in FabOS")

    def create_physical_payment(self, *, payment_id, amount_cents, currency, source_id, metadata):
        if not source_id:
            raise PaymentProviderError("Square source_id is required")
        ...
        try:
            ...
            raise PaymentProviderError("Square payment request failed") from exc
    ...
    def parse_webhook(self, payload, signature=None):
        if not self.webhook_signature_key or not self.webhook_url:
            raise PaymentProviderNotConfigured("SQUARE_WEBHOOK_SIGNATURE_KEY and SQUARE_WEBHOOK_URL are required")
        ...
        if not signature or not hmac.compare_digest(expected, str(signature)):
            raise PaymentProviderError("Invalid Square webhook signature")
        event = json.loads(payload.decode("utf-8") if isinstance(payload, bytes) else payload)
        event_type = str(event.get("type") or "")
        obj = ((event.get("data") or {}).get("object") or {})
        ...
        elif event_type in {"payment.failed"}: status = "failed"
        elif event_type in {"refund.created", "refund.updated"} and str(payment.get("status") or "").upper() == "REFUNDED": status = "refunded"
        return {"event_id": str(event.get("event_id") or ""), "event_type": event_type, "provider_payment_id": str(payment.get("id") or ""), "payment_id": str(payment.get("reference_id") or ""), "order_id": str(payment.get("reference_id") or ""), "status": status}


def base64_hmac_sha256(secret, message):
    import base64
...
class PaymentService:
    ...
    def _ensure_schema(self):
        with self.database.connect() as conn:
            conn.execute("""CREATE TABLE IF NOT EXISTS payment_transactions(
                ...
                amount_cents INTEGER NOT NULL, currency TEXT NOT NULL DEFAULT 'USD', provider TEXT NOT NULL,
                provider_payment_id TEXT, checkout_url TEXT, status TEXT NOT NULL DEFAULT 'created', metadata_json TEXT NOT NULL DEFAULT '{}',
            ...
            conn.execute("""CREATE TABLE IF NOT EXISTS payment_webhook_events(
                id TEXT PRIMARY KEY, provider TEXT NOT NULL, event_type TEXT, payment_id TEXT,
    ...
    def create_checkout(self, user_id, order_id):
        ...
        payment_id, amount_cents, metadata = self._new_transaction(order, customer["id"], invoice_id, provider.name, "customer-web")
    ...
    def record_physical_payment(self, order_id, source_id, provider_name="square"):
        ...
        if existing and str(existing["status"] or "").lower() not in {"failed", "cancelled"}: return self._json(existing)
        invoice_id, _ = self.invoices.create_from_order(order_id)
        payment_id, amount_cents, metadata = self._new_transaction(order, order["customer_id"], invoice_id, provider.name, "physical")
    ...
    def _update_gateway_fields(self, payment_id, result, status):
        with self.database.connect() as conn:
            conn.execute("UPDATE payment_transactions SET provider_payment_id=?,checkout_url=?,status=?,metadata_json=?,updated_at=CURRENT_TIMESTAMP WHERE id=?", (result.get("provider_payment_id"), result.get("checkout_url"), status, json.dumps(result.get("metadata") or {}, sort_keys=True), payment_id)); conn.commit()
