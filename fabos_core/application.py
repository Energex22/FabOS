...
from fabos_core.services.production_automation import ProductionAutomationService
...
class FabOSApplication:
    ...
    def _recover_previous_session(self):
        ...
        try:
            ...
            if recovered:self.error_log.warning("Recovered active print jobs after unclean shutdown","%d job(s) reconciled"%len(recovered))
        except Exception as exc:self.error_log.error("Crash recovery failed",exc)

    def _install_default_cura_profile(self):
        try:
            bundled=Path(__file__).resolve().parents[1]/"data"/"cura_profiles"/"Vyper PETG.curaprofile"; installed=self.cura.install_profile(bundled); saved=self.inventory_profit.setting("cura_petg_profile_path","")
            if not saved or not Path(saved).exists():self.inventory_profit.set_setting("cura_petg_profile_path",str(installed))
            if not self.inventory_profit.setting("default_slicer",""):self.inventory_profit.set_setting("default_slicer","Cura")
        except Exception:pass

    def summary(self):
        try:threshold=float(self.shop_settings.get('filament_low_threshold_g','250') or 250)
        except Exception:threshold=250
        with self.database.connect() as c:return {'products':c.execute('SELECT COUNT(*) FROM products').fetchone()[0],'orders':c.execute("SELECT COUNT(*) FROM orders WHERE status NOT IN ('completed','cancelled')").fetchone()[0],'queued_jobs':c.execute("SELECT COUNT(*) FROM print_jobs WHERE status IN ('queued','scheduled')").fetchone()[0],'printers':c.execute('SELECT COUNT(*) FROM printers').fetchone()[0],'low_spools':c.execute('SELECT COUNT(*) FROM filament_spools WHERE active=1 AND remaining_g<?',(threshold,)).fetchone()[0]}
